...
x_radius = x_radius * node_num
print('x radius: ' + str(x_radius))
y_radius = get_val_from_input_array(param_input, "y_radius")  # meter
y_radius = y_radius * node_num
...
top_log = open('topo_graph_density', 'a')
top_log.write('pos'+'-net'+str(node_num)+'-lqr'+str(rx_success)+'-it'+str(itr_num)+'-radius'+str(x_radius/node_num)+'='+ msf_pos_array +'\n')
top_log.close()
...
top_log_id = open('topo_graph_density', 'a')
top_log_id.write('nid-crit'+'-net'+str(node_num)+'-lqr'+str(rx_success)+'-it'+str(itr_num)+'-radius'+str(x_radius/node_num)+'='+str(critic_id_list)+'\n')
top_log_id.close()
...
top_log_id = open('topo_graph_density', 'a')
top_log_id.write('nid-bkg'+'-net'+str(node_num)+'-lqr'+str(rx_success)+'-it'+str(itr_num)+'-radius'+str(x_radius/node_num)+'='+str(bkg_id_list)+'\n')
top_log_id.close()
###################################################
...
def update_rand_seed():
    ...
    top_log_seed = open('topo_graph_density', 'a')
...
file_w = open("/home/fvg/contiki-ng/examples/sdn_udp/density-config-creator/config.csc", "w+")
...
newPath = shutil.copy('/home/fvg/contiki-ng/examples/sdn_udp/density-config-creator/config.csc', '/home/fvg/contiki-ng/examples/sdn_udp')
print("====> CONFIG-SDN file is created and added to .../sdn_udp/ directory")

newPath = shutil.copy('/home/fvg/contiki-ng/examples/sdn_udp/topo_graph_density', '/home/fvg/msf-contiki/contiki-ng/examples/6tisch/msf')
