      ...
      */
        ...
        for(j=0; j<MAX_SUBTREE; j++) {
          ...
          if(!linkaddr_cmp(&subtree_list[j].addr, &linkaddr_null)) {
            ...
            req_id_ch->is_ctrl = 0;
            linkaddr_cmp(&req_id_ch->req_sender, &subtree_list[j].addr);
            ...
            if(ee != NULL) {
              ...
              if((req_spec_ch->first_cell_down = extract_last_rxcell_of_hop(&ee->packet_info, req_id_ch, req_spec, &e->req_id.req_sender)) > -1) {
                printf("addr1 %d%d", req_id_ch->req_sender.u8[0], req_id_ch->req_sender.u8[1]);
                printf("addr2 %d%d", req_spec_ch->flow_id.u8[0], req_spec_ch->flow_id.u8[1]);
                printf("addr3 %d%d", req_spec_ch->src_addr.u8[0], req_spec_ch->src_addr.u8[1]);
                printf("addr4 %d%d", req_spec_ch->dest_addr.u8[0], req_spec_ch->dest_addr.u8[1]);
